...
function study_conditions=pseudorandomization(conditions,repeat_limit,number_trials_per_condition)
    
    number_conditions=prod(cellfun(@(x) length(x),conditions)); %calculate the number of unique conditions
    ...
    conditions_trials_original=strings(1,number_conditions); %initialize the vector
    num_iterated=0;
    for i=1:length(conditions{1})
        for j=1:length(conditions{2})
            num_iterated=num_iterated+1;
            conditions_trials_original(num_iterated)=strcat(conditions{1}(i),'-',conditions{2}(j));
        end
    end
    conditions_trials_original=repmat(conditions_trials_original,1,number_trials_per_condition); %create an original vector of all trials (not randomized)

    ncs_quality1=zeros(1,length(conditions{1})); %initialize the counting of consecutive trials with quality1 condition
    ncs_quality2=zeros(1,length(conditions{2})); %initialize the counting of consecutive trials with quality2 condition
    ...
    while i<number_trials_total %each loop randomly picks a trial from conditions_trials and assigns it to study_conditions
        ...
        while ~good %while the requirement is not satisfied, keep trying to randomly pick a trial that would satisfy our requirement
            ...
            conditions_iterated=strsplit(temp,'-'); %find which conditions from quality 1 and quality 2 were used in this picked trial
            
            temp_ncs_quality1=zeros(1,length(conditions{1})); %re-initialize temp_ncs
            temp_ncs_quality2=zeros(1,length(conditions{2}));
            temp_ncs_quality1(conditions{1}==conditions_iterated(1))=ncs_quality1(conditions{1}==conditions_iterated(1))+1; %add 1 to the condition of quality1 that is being re-iterated in this newly picked trial
            temp_ncs_quality2(conditions{2}==conditions_iterated(2))=ncs_quality2(conditions{2}==conditions_iterated(2))+1;

            if ~(any(temp_ncs_quality1>repeat_limit(1)) || any(temp_ncs_quality2>repeat_limit(2))) %check whether with this newly picked trial, we are failing the requirement
                good=1; %if not failing the requirement, this trial is good, we're keeping it
                ncs_quality1=temp_ncs_quality1; %update ncs_quality1
                ncs_quality2=temp_ncs_quality2;
                ...
                if num_repeats>10 %if this is is more than the 10th time you're trying to pick a trial from the remaining trials, it is likely that there are not trials left that would satisfy our requirement- restart randomization
                    ...
                    ncs_quality1=zeros(1,length(conditions{1})); %re-initialize ncs_quality1
                    ncs_quality2=zeros(1,length(conditions{2}));
